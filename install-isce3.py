#!/usr/bin/env python3
# Portable shebang - uses first python3 in PATH
...
def get_defaults(instance_type: str) -> argparse.Namespace:
    ...
    defaults.fetch = None  # None means no additional refspec to fetch
    defaults.merge = True
    defaults.commit_hash = None  # None means use version instead
...
def main() -> None:
    ...
    if OPTIONS.commit_hash is not None:
        print(f"Checking out commit {OPTIONS.commit_hash}")
    ...
    #env_def_file = os.path.join(OPTIONS.repo_dir, 'tools', 'imagesets', 'oracle8conda', 'runtime', 'spec-file.txt')
    env_def_file = os.path.join(OPTIONS.repo_dir, 'tools', 'imagesets', 'oracle8conda', 'dev', 'spec-file.txt')
...
def get_options() -> argparse.Namespace:
    ...
    repo_group.add_argument(
        '--fetch',
        metavar='REFSPEC',
        help="fetch an additional refspec from origin (always fetches all refs first). "
             "For pull requests, provide the PR number or full refspec. "
             "After fetching, use --version FETCH_HEAD to check out the fetched ref. "
             "Examples: --fetch 123 (fetches all refs + PR #123), "
             "--fetch pull/123/head (equivalent), --fetch refs/tags/v1.0 (specific tag). "
             "Complete workflow: --fetch 123 --version FETCH_HEAD")
    repo_group.add_argument(
        '--merge',
    ...
    repo_group.add_argument(
        '--commit-hash',
        help="specific commit hash to checkout (overrides --version if both specified)")
...
def fetch_refs(additional_refspec: str | None = None) -> None:
    """Fetch refs from the ISCE3 repository.

    Always fetches all refs from all remotes.
    If additional_refspec is provided, also fetch that specific refspec from origin.

    Args:
        additional_refspec: Optional additional refspec to fetch (e.g., 'pull/123/head' for PR #123, or just '123')
    """
    # Always fetch all refs first
    args = [OPTIONS.git_exec, 'fetch', '--all']
    run_command(args, "error: fetching all refs from the ISCE3 Git repo failed", cwd=OPTIONS.repo_dir)

    # If additional refspec provided, fetch it from origin
    if additional_refspec is not None:
        # Auto-expand PR numbers to full refspec for convenience
        if additional_refspec.isdigit():
            additional_refspec = f'pull/{additional_refspec}/head'
        args = [OPTIONS.git_exec, 'fetch', 'origin', additional_refspec]
        run_command(args, f"error: fetching refspec '{additional_refspec}' from origin failed", cwd=OPTIONS.repo_dir)
...
def checkout_commit_hash() -> None:
    """Check out specific commit hash in ISCE3 repository."""
    args = [OPTIONS.git_exec, 'checkout', OPTIONS.commit_hash]
    run_command(args, "error: checking out commit hash failed", cwd=OPTIONS.repo_dir)
...
def create_env(env_def_file: pathlib.Path) -> None:
    ...
    if False:
        # Original static enviroment file
        ...
        # For creating directly from the ISCE3 file
        args = [OPTIONS.mamba_exec, 'create', '--file', env_def_file, '-p', OPTIONS.env_prefix, '--yes']
        run_command(args, "error: creating the ISCE3 Conda environment failed")
        args = [OPTIONS.mamba_exec, 'install', '-p', OPTIONS.env_prefix, '-c', 'conda-forge', '--yes', 'ipykernel']
        run_command(args, "error: creating the ISCE3 Conda environment failed")
        # There is no CMAKE or g++ in this file...............
        # args = [OPTIONS.mamba_exec, 'install', 'cmake', 'gxx_linux-64==11.2', '-p', OPTIONS.env_prefix, '--yes']
        # run_command(args, "error: creating the ISCE3 Conda environment failed")
    else:
        # To address RAIDER having isce3 as a dependency
        args = [OPTIONS.mamba_exec, 'create', '-p', OPTIONS.env_prefix, '--yes', '--no-default-packages']
        run_command(args, "error: creating the ISCE3 Conda environment failed")
        args = [OPTIONS.mamba_exec, 'install', '--file', env_def_file, '-p', OPTIONS.env_prefix, '--yes', '--no-deps']
        run_command(args, "error: creating the ISCE3 Conda environment failed")


def add_env() -> None:
    """Update the ipykernel to recognize the ISCE3 Conda environment."""
    args = [OPTIONS.env_prefix / 'bin' / 'python', '-m', 'ipykernel', 'install', '--user', '--name', OPTIONS.env_name, '--display-name', OPTIONS.env_name]
    run_command(args, "error: updating the ipykernel to add the ISCE3 Conda environment failed", env_prefix=OPTIONS.env_prefix)

    # 1. Resolve the path to the kernel.json file created by the --user flag
    # Note: On Linux/Unix, --user installs to ~/.local/share/jupyter/kernels
    kernel_dir = pathlib.Path.home() / '.local' / 'share' / 'jupyter' / 'kernels' / OPTIONS.env_name
    kernel_json = kernel_dir / 'kernel.json'

    # 2. Define the kernda arguments
    # -o: overwrite the existing file
    # -y: assume "yes" to prompts
    kernda_args = [
        'kernda', '-o', '-y',
        '--env-dir', str(OPTIONS.env_prefix),
        str(kernel_json)
    ]

    # 3. Run the command using your wrapper
    run_command(
        kernda_args,
        "error: failed to patch the kernel with kernda to activate environment variables",
