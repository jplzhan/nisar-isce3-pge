...
def apply_tls(obj, verify) -> None:
    """Set ``obj._session.verify`` when ``verify`` is a real bundle path.

    Works uniformly for any otello object (Mozart, CI, Job, ...) since all share a
    requests Session on ``_session``. When ``verify`` is None the session is left
    as otello configured it (insecure) -- the caller handles that case explicitly.
    """
    if verify is None:
        return
